...
const SpaBookingStatus: React.FC<SpaBookingStatusProps> = ({ 
  ...
  const steps = [
    ...
    { id: 2, name: t('spa.preparation'), icon: <Bath className="h-5 w-5" />, complete: isToday },
    { id: 3, name: t('spa.ready'), icon: <Sparkles className="h-5 w-5" />, complete: isToday && hoursRemaining <= 1 }
  ];
          ...
          {timeRemaining > 0 ? (
            <div className="bg-muted/30 p-3 rounded-md text-sm">
              ...
              <p className="text-muted-foreground pl-6">
                {hoursRemaining > 0 ? `${hoursRemaining} ${t('spa.hours')} ${t('spa.and')} ` : ''}
                {minutesRemaining} {t('spa.minutes')} {t('spa.remaining')}
              </p>
            </div>
          ) : (
