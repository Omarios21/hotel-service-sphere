...
const ActivityBookingStatus: React.FC<ActivityBookingStatusProps> = ({ 
  ...
  const steps = [
    ...
    { id: 3, name: t('status.ready'), icon: <Trophy className="h-5 w-5" />, complete: isToday && hoursUntil <= 1 }
  ];
        ...
        <CardContent>
          ...
          {timeUntilActivity > 0 ? (
            <div className="bg-muted/30 p-3 rounded-md text-sm">
              ...
              <p className="text-muted-foreground pl-6">
                {hoursUntil > 0 ? `${hoursUntil} ${t('activities.hours')} ${t('roomService.and')} ` : ''}
                {minutesUntil} {t('activities.minutes')} {t('roomService.betweenTimes')}
              </p>
            </div>
          ) : (
